...
fn test_beam_lattice_requires_extension() {
    ...
    if let Ok(file) = file {
        ...
        if result_with_extension.is_ok() {
            ...
            if result_core.is_err() {
                match result_core.unwrap_err() {
                    Error::UnsupportedExtension(msg) => {
                        println!("✓ Correctly rejected file without BeamLattice extension: {}", msg);
                    }
                    _ => {
            ...
            println!("Note: Could not parse beam lattice file: {:?}", result_with_extension.err());
        }
    } else {
    ...
    if let Ok(file) = file {
        ...
        if result_with_extension.is_ok() {
            ...
            if result_core.is_err() {
                match result_core.unwrap_err() {
                    Error::UnsupportedExtension(msg) => {
                        println!("✓ Correctly rejected file without Production extension: {}", msg);
                    }
                    _ => {
                        println!("✓ File failed to parse without Production extension (different error)");
                    }
                }
            } else {
            ...
            println!("Note: Could not parse production file: {:?}", result_with_extension.err());
        }
    } else {
...
fn test_core_file_works_without_extensions() {
    ...
    if let Ok(file) = file {
        ...
        match result {
            ...
            Err(e) => {
                panic!("Core file should parse with core-only config, but failed: {:?}", e);
            }
        }
    } else {
        ...
        if result_with_extension.is_ok() {
            ...
            if result_core.is_err() {
                match result_core.unwrap_err() {
                    ...
                    _ => {
                        println!("✓ File failed to parse without Slice extension (different error)");
                    }
                }
            } else {
            ...
            println!("Note: Could not parse slice file: {:?}", result_with_extension.err());
        }
    } else {
