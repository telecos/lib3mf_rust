...
fn test_texture_path_validation_accepts_singular() {
    ...
    assert!(result.is_ok(), "File with /3D/Texture/ path should parse successfully: {:?}", result.err());
    
    let model = result.unwrap();
    assert_eq!(model.resources.texture2d_resources.len(), 1, "Should have 1 texture");
    assert!(
        model.resources.texture2d_resources[0].path.to_lowercase().starts_with("/3d/texture/"),
...
fn test_all_pxxm_0508_files() {
    ...
    for i in 1..=14 {
        let test_file = format!("test_suites/suite6_core_matl/positive_test_cases/P_XXM_0508_{:02}.3mf", i);
