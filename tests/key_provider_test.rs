...
use lib3mf::{AccessRight, CEKParams, KEKParams, KeyProvider, ParserConfig, Result, SecureContentInfo};
use std::sync::Arc;
...
fn test_custom_provider_decryption() {
    ...
    assert!(result.is_ok(), "Failed to parse with custom provider: {:?}", result.err());
    
    let model = result.unwrap();
    
    // Verify that secure content was processed
    assert!(model.secure_content.is_some(), "SecureContent info should be present");
}
...
fn test_fallback_to_test_keys() {
    ...
    assert!(result.is_ok(), "Failed to parse with test keys: {:?}", result.err());
...
fn test_custom_provider_is_called() {
    ...
    assert!(result.is_err(), "Should have failed with custom provider error");
    
    let error = result.unwrap_err();
    let error_msg = format!("{}", error);
    
    // Verify it was our custom provider that was called
    assert!(error_msg.contains("Custom provider was called"),
        "Error should be from custom provider, got: {}", error_msg);
}
