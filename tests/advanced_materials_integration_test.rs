...
fn test_parse_and_write_texture2d() {
    ...
    let mut texture = Texture2D::new(1, "/3D/Textures/wood.png".to_string(), "image/png".to_string());
...
fn test_parse_and_write_composite_materials() {
    ...
    base_group.materials.push(BaseMaterial::new("Blue".to_string(), (0, 0, 255, 255)));
    ...
    comp_group.composites.push(Composite::new(vec![0.5, 0.3, 0.2])); // Purple-ish
    comp_group.composites.push(Composite::new(vec![0.7, 0.2, 0.1])); // More red
    comp_group.composites.push(Composite::new(vec![0.1, 0.8, 0.1])); // Mostly blue
