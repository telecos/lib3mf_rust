...
use lib3mf::{assemble_contours, collect_intersection_segments, Mesh, Model, Object};
use std::path::Path;
use thiserror::Error;
...
impl Slicer {
    ...
    pub fn slice_model(
        ...
        println!("  Generating {} layers from Z={:.2} to Z={:.2} mm", num_layers, z_min, z_max);
            ...
            for mesh in &meshes {
                ...
                if !segments.is_empty() {
                    ...
                    for contour in contours {
                        let points: Vec<Point2D> = contour
                            .iter()
        ...
        println!("Slicing complete! Generated {} slice images.", output_files.len());
    ...
    fn extract_meshes(&self, model: &Model) -> Result<Vec<Mesh>, SlicerError> {
        ...
        let meshes_enabled = spec_support.map_or(true, |s| s.meshes);
...
mod tests {
    use super::*;
    use crate::config::{Point3D, Resolution};
    ...
    fn test_slicer_creation() {
        let config = SlicerConfig {
            ...
            printable_box: PrintableBox {
                ...
                end: Point3D { x: 200.0, y: 200.0, z: 200.0 },
            },
