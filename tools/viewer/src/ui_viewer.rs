...
fn apply_render_mode_to_node(node: &mut SceneNode, mode: RenderMode) {
    match mode {
        RenderMode::Solid => {
            ...
            // Clear any wireframe overlay colors
        ...
        RenderMode::XRay => {
            node.set_surface_rendering_activation(true);
            node.set_lines_width(1.0); // Add subtle wireframe for X-Ray effect
            node.set_points_size(0.0);
            // Set a lighter wireframe color to simulate transparency
            node.set_lines_color(Some(Point3::new(0.3, 0.3, 0.3)));
            // Note: kiss3d has limited transparency support, so we simulate X-Ray
            // by adding wireframe overlay with lighter color to create a see-through effect
        }
    }
}
