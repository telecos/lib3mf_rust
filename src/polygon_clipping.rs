...
fn slice_polygon_to_paths(
    ...
    if polygon.startv >= vertices.len() {
        return Err(ClippingError::InvalidPolygon(
            format!("Start vertex index {} out of bounds", polygon.startv)
        ));
    }
    ...
    for segment in &polygon.segments {
        if segment.v2 >= vertices.len() {
            return Err(ClippingError::InvalidPolygon(
                format!("Segment vertex index {} out of bounds", segment.v2)
            ));
        }
...
fn paths_to_slice_polygons(
    ...
    for path in paths {
        ...
        for point in path.iter().skip(1) {
            ...
            polygon.segments.push(crate::model::SliceSegment::new(vertex_idx));
        }
...
mod tests {
    ...
    fn test_intersection_two_squares() {
        ...
        assert!(!result.is_empty(), "Expected at least one polygon in intersection");
    }
    ...
    fn test_difference_two_squares() {
        ...
        let mut result_vertices = Vec::new();
        let result = difference_polygons(
            &[polygon1],
            &[polygon2],
            &vertices,
            &mut result_vertices
        ...
        assert!(!result.is_empty(), "Expected at least one polygon in difference");
    }
