                ...
                match local_name {
                    ...
                    "basematerials" if in_resources => {
                        ...
                        let id = attrs
                            .get("id")
                            .ok_or_else(|| {
                                Error::missing_attribute("basematerials", "id")
                            })?
                            .parse::<usize>()?;
                    ...
                    "colorgroup" if in_resources => {
                        ...
                        let id = attrs
                            .get("id")
                            .ok_or_else(|| {
                                Error::missing_attribute("colorgroup", "id")
                            })?
                            .parse::<usize>()?;
                    ...
                    "texture2d" if in_resources => {
                        ...
                        let id = attrs
                            .get("id")
                            .ok_or_else(|| {
                                Error::missing_attribute("texture2d", "id")
                            })?
                            .parse::<usize>()?;
                        let path = attrs
                            .get("path")
                            .ok_or_else(|| {
                                Error::missing_attribute("texture2d", "path")
                            })?
                            .to_string();
                    ...
                    "texture2dgroup" if in_resources => {
                        ...
                        let id = attrs
                            .get("id")
                            .ok_or_else(|| {
                                Error::missing_attribute("texture2dgroup", "id")
                            })?
                            .parse::<usize>()?;
                        let texid = attrs
                            .get("texid")
                            .ok_or_else(|| {
                                Error::missing_attribute("texture2dgroup", "texid")
                            })?
                            .parse::<usize>()?;
                    ...
                    "tex2coord" if in_texture2dgroup => {
                        if let Some(ref mut group) = current_texture2dgroup {
                            ...
                            let u = attrs
                                .get("u")
                                .ok_or_else(|| {
                                    Error::missing_attribute("tex2coord", "u")
                                })?
                                .parse::<f32>()?;
                            let v = attrs
                                .get("v")
                                .ok_or_else(|| {
                                    Error::missing_attribute("tex2coord", "v")
                                })?
                                .parse::<f32>()?;
