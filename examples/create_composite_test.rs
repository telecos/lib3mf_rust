...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    comp_group.composites.push(Composite::new(vec![0.7, 0.3, 0.0])); // Mostly red + some green = Orange
    comp_group.composites.push(Composite::new(vec![0.0, 0.6, 0.4])); // Green + blue = Teal
    comp_group.composites.push(Composite::new(vec![0.5, 0.0, 0.5])); // Red + blue = Purple
