...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_files = vec![
        ...
        ("Material/Kinect Scan", "test_files/material/kinect_scan.3mf"),
        ("Production/Box", "test_files/production/box_prod.3mf"),
        ("Slice/Box Sliced", "test_files/slices/box_sliced.3mf"),
        ("Beam Lattice/Pyramid", "test_files/beam_lattice/pyramid.3mf"),
    ];
                    ...
                    Ok(model) => {
                        ...
                        for obj in &model.resources.objects {
                            ...
                            if let Some(ref mesh) = obj.mesh {
                                ...
                                let with_pid = mesh.triangles.iter()
                                    .filter(|t| t.pid.is_some()).count();
                                if with_pid > 0 {
