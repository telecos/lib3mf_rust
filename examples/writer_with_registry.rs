...
fn main() -> Result<()> {
    ...
    let file = File::create("/tmp/example_with_default_registry.3mf")?;
    model.clone().to_writer_with_registry(file, &registry)?;
    println!("   ✓ Written to /tmp/example_with_default_registry.3mf\n");
    ...
    let file = File::create("/tmp/example_with_custom_registry.3mf")?;
    model.clone().to_writer_with_registry(file, &custom_registry)?;
    println!("   ✓ Written to /tmp/example_with_custom_registry.3mf\n");
    ...
    let file = File::create("/tmp/example_without_registry.3mf")?;
    model.clone().to_writer(file)?;
    println!("   ✓ Written to /tmp/example_without_registry.3mf\n");

    // Method 4: Using write_to_file_with_registry
    println!("5. Using write_to_file_with_registry convenience method...");
    model
        .clone()
        .write_to_file_with_registry("/tmp/example_convenience.3mf", &custom_registry)?;
    println!("   ✓ Written to /tmp/example_convenience.3mf\n");

    // Verify the files can be read back
    println!("6. Verifying written files can be read back...");
    let model1 = Model::from_reader(File::open("/tmp/example_with_default_registry.3mf")?)?;
    let model2 = Model::from_reader(File::open("/tmp/example_with_custom_registry.3mf")?)?;
    let model3 = Model::from_reader(File::open("/tmp/example_without_registry.3mf")?)?;
    let model4 = Model::from_reader(File::open("/tmp/example_convenience.3mf")?)?;
    ...
    // Clean up
    std::fs::remove_file("/tmp/example_with_default_registry.3mf").ok();
    std::fs::remove_file("/tmp/example_with_custom_registry.3mf").ok();
    std::fs::remove_file("/tmp/example_without_registry.3mf").ok();
    std::fs::remove_file("/tmp/example_convenience.3mf").ok();
